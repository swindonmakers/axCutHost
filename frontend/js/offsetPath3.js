...
// returns an array of points that form a flattened curve
function flattenCurve(cp, tol) {
	var res = new Array();
	if (Curve.isFlatEnough(cp,tol)) {
		res.push(Curve.evaluate(cp,1,0));
	} else {
		// divide
		var cp = Curve.subdivide(cp, 0.5);
		
		var r1 = flattenCurve(cp[0],tol);
		var r2 = flattenCurve(cp[1],tol);
...
function flattenPath(p, tol) {
	...
	while(i<p.curves.length) {
		...
		if (c) {
			if (c.isLinear() ) {
				...
				var points = flattenCurve(c.getValues(),tol);
	...
	p.removeSegments();
	p.addSegments(np.segments);
	np.remove();
	np = null;

	//drawPathPoints(p, 'blue');
}
...
function createJoint(c1, c2, pc, radius) {
	...
	// if both curves are linear, then will attempt to trim them
	
	var p1, ps1;
	if (c1.type =='path') {
		p1 = c1.lastSegment.point;
		ps1 = c1.segments[c1.segments.length-2].point;
	} else {
		p1 = c1.point2;
		ps1 = c1.point1;
	}
	...
	if (p1 == p2) {
		// coincident endpoints, nothing to be done!
		...
		if (v.length < radius/100) {
			// endpoints are really close - so align them
		...
		/*else if (
			...
			// there's a gap, so need to either join with a curve, or trim the ends
			...
			if (ang < 0) {
				...
				if (intersec.onLine1 && intersec.onLine2) {
				  ...
				  // align endpoints to intersection
				  p1.x = intersec.x;
				  p1.y = intersec.y;
				  p2.x = intersec.x;
				  p2.y = intersec.y;
			...
			if (ang <= 0 || ang >= 180) {
				//ang = -ang;
				// rare and annoying - needs a straight line joint
				return null;
			}
...
function getNearestPointBetweenPaths(p1,p2,radius,tol,sg,og) {
	...
	// for each curve in p1
	for (var i=0; i<p1.curves.length; i++) {
		var c = p1.curves[i];
		
		// just check the ends for speed...
		if (i > 5 && i < p1.curves.length-5) continue;

		// lookup neighbours of curve c in source grid
		var neigh = getGridNeighbours(sg, c);
	
		// check for min distance to each neighbour
		for (var j=0; j<neigh.length; j++) {
			var n = neigh[j];
			
			// test from curve c to neighbour point1
			var np = c.getNearestLocation(n.point1);
			var distToP = np.point.subtract(n.point1).length;
			
			if (distToP < nearestPointDist && nearestPointDist > 0) {
				nearestPointDist = distToP;
				nearestPoint = np.point;
				nearestPointSrc = n.point1;
			}
		
			if ((nearestPointDist < radius - tol) && nearestPointDist > 0) {
				break;
			}
			
			// test from curve c to neighbour point2			
			var np = c.getNearestLocation(n.point2);
			var distToP = np.point.subtract(n.point2).length;
		
			if (distToP < nearestPointDist && nearestPointDist > 0) {
				nearestPointDist = distToP;
				nearestPoint = np.point;
				nearestPointSrc = n.point2;
			}
			...
			// now test from neighbour to c point 1
			var np = n.getNearestLocation(c.point1);
			var distToP = np.point.subtract(c.point1).length;
		
			if (distToP < nearestPointDist && nearestPointDist > 0) {
				nearestPointDist = distToP;
				nearestPoint = np.point;
				nearestPointSrc = c.point1;
			}
			...
			// finally test from neighbour to c point 2
			var np = n.getNearestLocation(c.point2);
			var distToP = np.point.subtract(c.point2).length;
		
			if (distToP < nearestPointDist && nearestPointDist > 0) {
				nearestPointDist = distToP;
				nearestPoint = np.point;
				nearestPointSrc = c.point2;
			}
	...
	// debug
	if (false && nearestPointDist < radius - tol && nearestPointDist > 0) {
		var sc = new Color(Math.random(),Math.random(),Math.random(), 1);
...
function calcOffsetPath(p, radius, progressCallback, stopAfter) {
	...
	// setup grids
	var og = makeGrid(p.bounds, radius, p.curves.length);
	var sg = makeGrid(p.bounds, radius, p.curves.length);
	...
	for (var i=0; i<cc; i++) {
		...
		if (op.curves.length > 0) {
			...
			if (jc && jc!=-1) {
				...
				// if no joint created, then link to last segment via straight line
				// should only do this if the end-points are not co-incident!
	...
	// populate offset grid, id curves
	var id = 0;
	...
	console.log("children: "+op.children.length);
	...
	if (progressCallback) progressCallback('start','Evaluating self intersections');
	
	console.log('Evaluating self intersections');
	
	var intersections = [];
	
	var checked = new Array(id);  // src to dest
	...
	while (i<op.curves.length) {
		...
		var neigh = getGridNeighbours(og,c1);
		...
		for (var j = 0; j< neigh.length; j++) {
			...
			var check = checked[c1.id];
			if (check && check.length && check.length > 0 && check[c2.id]) continue;
			
			if (!checked[c1.id]) checked[c1.id] = new Array(id);
			checked[c1.id][c2.id] = 1;
	...
	console.log("children pre-split: "+op.children.length);
	...
	console.log("children after splits: "+op.children.length);
	
	
	console.log('Joining close points');
	// merge close points
	console.log('Merged: '+joinCloseSegments(op, radius/40));
	
	console.log('Removing unwanted paths');
	if (progressCallback) progressCallback('start','Clipping');
	
	
	// rebuild offset grid after splitting and tidying
	og = makeGrid(p.bounds, radius, p.curves.length);
	for (var i=0; i<op.curves.length; i++) {
		var c = op.curves[i];
		addToGrid(og, c);
	}
	...
	while (i<op.children.length && op.children.length > 0) {
		...
		var tol = radius/40;
		var nearestPoint = getNearestPointBetweenPaths(op1, p, radius, tol, sg,og);
		
		
		if (nearestPoint.dist && (radius - nearestPoint.dist) > tol) {
			removeChild = true;
		} else {
...
function analyseSelfIntersections(p, radius) {
	...
	var checked = new Array(p.curves.length);  // src to dest
	...
	while (i<p.curves.length) {
		...
		// get neighbours from grid
		var neigh = getGridNeighbours(sg,c1);
		...
		for (var j = 0; j< neigh.length; j++) {
			...
			if (!checked[c1.id]) checked[c1.id] = new Array(p.curves.length);
			checked[c1.id][c2.id] = 1;
			...
			if (c1.bounds.intersects(c2.bounds)) {
				...
				for (var k=0; k<ci.length; k++) {
					...
					debugCrossHair(intersec.point,1,'red');
					
					var c1p1 = intersec.point.getDistance(c1.point1,true);
					var c1p2 = intersec.point.getDistance(c1.point2,true);
					
					if (c1p1 == 0) {
						c1.point1.x += (Math.random()-0.5) * radius/10;
						c1.point1.y += (Math.random()-0.5) * radius/10;
					}
					if (c1p2 == 0) {
						c1.point2.x += (Math.random()-0.5) * radius/10;
						c1.point2.y += (Math.random()-0.5) * radius/10;
					}
					...
					if (c2p1 == 0) {
						c2.point1.x += (Math.random()-0.5) * radius/10;
						c2.point1.y += (Math.random()-0.5) * radius/10;
					}
					if (c2p2 == 0) {
						c2.point2.x += (Math.random()-0.5) * radius/10;
						c2.point2.y += (Math.random()-0.5) * radius/10;
					}
