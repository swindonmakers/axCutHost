	...
	split: function(index, parameter) {
		...
		if (index >= 0 && index < curves.length) {
			...
			if (this._closed) {
				...
				//path = this._clone(new Path().insertBelow(this, true));
			}
			path._add(segs, 0);
...
new function() {
	...
	function importGroup(node, type, isRoot, options) {
		...
		if (!isClip) {
			...
			// inkscape extensions
			if (node.hasAttribute('inkscape:groupmode')) {
				item.inkscapeGroupmode = node.getAttribute('inkscape:groupmode');
			}
			if (node.hasAttribute('inkscape:label')) {
				item.inkscapeLabel = node.getAttribute('inkscape:label');
			}
		}
